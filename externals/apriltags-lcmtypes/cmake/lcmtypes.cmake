...
function(lcmtypes_build_c)
    ...
    set(modewords CPP_LIBNAME C_AGGREGATE_HEADER)
    set(curmode "")
    foreach(word ${ARGV})
...
function(lcmtypes_build_cpp)
    ...
    # library name
    set(libname "lcmtypes_cpp_${PROJECT_NAME}")

    # header file that includes all other generated header files
    set(agg_hpp_bname "${__sanitized_project_name}.hpp")

    # allow defaults to be overriden by function parameters
    set(modewords CPP_LIBNAME CPP_AGGREGATE_HEADER)
    set(curmode "")
    foreach(word ${ARGV})
        list(FIND modewords ${word} mode_index)
        if(${mode_index} GREATER -1)
        ...
        elseif(curmode STREQUAL CPP_AGGREGATE_HEADER)
            set(agg_h_bname "${word}")
            set(curmode "")
        elseif(curmode STREQUAL CPP_LIBNAME)
            set(libname "${word}")
            set(curmode "")
        endif()
    endforeach()
    ...
    lcmgen(--lazy -x --cpp-cpath ${_lcmtypes_cpp_dir} --cpp-hpath ${_lcmtypes_cpp_dir} --cpp-include lcmtypes ${_lcmtypes})

    # run lcm-gen at compile time
    add_custom_target(lcmgen_cpp ALL 
        COMMAND sh -c '[ -d ${_lcmtypes_cpp_dir} ] || mkdir -p ${_lcmtypes_cpp_dir}'
        COMMAND sh -c 'lcm-gen --lazy -x ${_lcmtypes} --cpp-cpath ${_lcmtypes_cpp_dir} --cpp-hpath ${_lcmtypes_cpp_dir}')

    # get a list of all generated .c and .h files
    file(GLOB_RECURSE _lcmtypes_cpp_files ${_lcmtypes_cpp_dir}/*.cpp)
    file(GLOB_RECURSE _lcmtypes_hpp_files ${_lcmtypes_cpp_dir}/*.hpp)

    include_directories(BEFORE ${PROJECT_SOURCE_DIR}/lcmtypes/cpp)
    include_directories(${LCM_INCLUDE_DIRS})
    include_directories(${PROJECT_SOURCE_DIR}/lcmtypes/cpp/lcmtypes)

    # aggregate into a static library
    add_library(${libname} STATIC ${_lcmtypes_cpp_files})
    set_source_files_properties(${_lcmtypes_cpp_files} PROPERTIES COMPILE_FLAGS "-fPIC")
    #    set_target_properties("${libname}-static" PROPERTIES OUTPUT_NAME "${libname}")
    set_target_properties(${libname} PROPERTIES PREFIX "lib")
    set_target_properties(${libname} PROPERTIES CLEAN_DIRECT_OUTPUT 1)
    add_dependencies(${libname} lcmgen_cpp)
    ...
    install(TARGETS ${libname} ARCHIVE DESTINATION lib)
    #install(TARGETS "${libname}" LIBRARY DESTINATION lib)
    install(FILES ${_lcmtypes_hpp_files} DESTINATION include/lcmtypes)

    # set some compilation variables
    set(LCMTYPES_INCLUDE_DIRS ${LCMTYPES_INCLUDE_DIRS} ${PROJECT_SOURCE_DIR}/lcmtypes/cpp PARENT_SCOPE)
    set(LCMTYPES_LIBS ${LCMTYPES_LIBS} ${libname} PARENT_SCOPE)
